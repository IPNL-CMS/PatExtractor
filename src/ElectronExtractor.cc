...
void ElectronExtractor::doMCMatch(const pat::Electron& object, MCExtractor* mcExtractor, int index) {
  ...
  for(int mcPart_i=0; mcPart_i<mcExtractor->getSize(); ++mcPart_i) 
  {
    if (mcExtractor->getStatus(mcPart_i)!=3) continue; //according to pat, electrons are matched to particles of status 1, but i have none matched unless is put 3
    if (fabs(mcExtractor->getType(mcPart_i))!=11) continue;
    TL_genPart.SetPxPyPzE(mcExtractor->getPx(mcPart_i),mcExtractor->getPy(mcPart_i),mcExtractor->getPz(mcPart_i),mcExtractor->getE(mcPart_i));
    TL_electron.SetPxPyPzE(object.px(), object.py(), object.pz(), object.energy());
    if(TL_genPart.Pt())
    {
      ...
      //      if((mcExtractor->getType(mcPart_i)*part.charge())<0.) continue;//doesnt work for now (i.e. noone matched) dont know why
